...
# registry.suse.com/suse/sle15:15.7 ships curl preinstalled (like Rocky, unlike Debian/Ubuntu) --
# the guard below is a no-op here, kept for consistency across all four OSes
# and to protect against a future base-image change.
step "Downloading RCC ..."
if ! command -v curl > /dev/null; then
  zypper --non-interactive install curl || fail "zypper install curl failed -- cannot download RCC."
fi
